...
const FRICTION = 0.9; //gravity (0 = no graviry, 1 = a lot of gravity)
const GAME_LIVES = 3; //Player lives
const SAVE_KEY_SCORE = "highscore"; //save key for local storage of high score

const LAZER_MAX = 10; //Max num of lazers on screen
const LAZER_DIST = 0.9; //Max distance lazer can travel
const LAZER_SPD = 500; //Speed of lazers (px/s)
...
const ROIDS_SPD = 40; //Max Starting speed of astroids
const ROIDS_SIZE = 100; //Starting size of astroids (px)
const ROIDS_VERT = 10; //Average number of vertisies on each astroid
const ROIDS_PTS_LGE = 10; //Large astroid = +20pts
const ROIDS_PTS_MED = 25; //Medium astroid = +50pts
const ROIDS_PTS_SML = 40; //Small astroid = +100pts

const SHIP__DUR = 5; //ship is invinsible for 5 sec after crash
const SHIP_BLINK_DUR = 0.1; //how fast ship blinks when revived
const SHIP_EXPLOAD_DUR = 0.7; //ship explosion length
const SHIP_INV_DUR = 5; //ship is invinsible for 5 sec after crash
...
const TURN_SPEED = 300; //Rotates 360deg per second
...
function destroyAstroid(index){
	...
	if(r == Math.ceil(ROIDS_SIZE / 2)){
		...
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 6))); //makes new astroid half its original size
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 6)));
		score += ROIDS_PTS_MED;
	}else{
		score += ROIDS_PTS_SML;
	}
...
function keyDown(ev){
	...
	switch(ev.keyCode){
		...
		case 40: //Down key (moves ship backwards)
			ship.reversing = true; //Is moving backwards
		break;

		case 83: //S key (moves ship Backwards)
			ship.reversing = true; //Is moving backwards
		break;
	}
}
...
function keyUp(ev){
	...
	switch(ev.keyCode){
		...
		case 40: //up key (moves ship foreward)
			ship.reversing = false; //Is moving backwards
		break;
...
function update(){
	...
	if(ship.reversing && !ship.dead){ //reverse ship
		ship.reverse.x -= SHIP_THRUST * Math.cos(ship.a) / FPS;
		ship.reverse.y -= SHIP_THRUST * Math.sin(ship.a) / FPS;
	}else{ //Slows down ship if youre not pressing the up key
		ship.reverse.x -= FRICTION * ship.reverse.x / FPS;
		ship.reverse.y -= FRICTION * ship.reverse.y / FPS;
	}
	...
	if(!exploading && !ship.dead){
		...
		//Move Space Ship Backwards
		ship.x += ship.reverse.x;
		ship.y -= ship.reverse.y;
	}else{
		ship.exploadTime--;
