...
const FRICTION = 0.1; //gravity (0 = no graviry, 1 = a lot of gravity)
const GAME_LIVES = 3; //Player lives
const SAVE_KEY_SCORE = "highscore"; //save key for local storage of high score

const LAZER_MAX = 10; //Max num of lazers on screen
const LAZER_DIST = 0.3; //Max distance lazer can travel
const LAZER_SPD = 400; //Speed of lazers (px/s)
const LAZER_EXPLOAD_DUR = 0.1; //lazer expload time

const ROIDS_JAG = 0.4; //uneveness of the astroid (0 = smooth, 1 = very uneven)
const ROIDS_NUM = 5; //Starting number of astroids
const ROIDS_SPD = 70; //Max Starting speed of astroids
const ROIDS_SIZE = 120; //Starting size of astroids (px)
...
const ROIDS_PTS_XXSML = 200; //EXTRA extra Small astroid = +150pts

const SHIP__DUR = 2; //ship is invinsible for 3 sec after crash
const SHIP_BLINK_DUR = 0.2; //how fast ship blinks when revived
const SHIP_EXPLOAD_DUR = 0.7; //ship explosion length
const SHIP_INV_DUR = 2; //ship is invinsible for 3 sec after crash
const SHIP_SIZE = 35; //30px
const SHIP_THRUST = 3; //5px/second when moving foreward (The longer you hold the key, the faster it moves)

const TURN_SPEED = 330; //Rotates 360deg per second
...
function destroyAstroid(index){
	...
	if(r == Math.ceil(ROIDS_SIZE / 2)){
		...
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12))); //makes new astroid half its original size
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12)));
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12))); //makes new astroid half its original size
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12)));
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12))); //makes new astroid half its original size
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12)));
		...
		roids.push(newAstroid(x, y, Math.ceil(ROIDS_SIZE / 12)));
		score += ROIDS_PTS_SML;
	}else if(r == Math.ceil(ROIDS_SIZE / 16)){
